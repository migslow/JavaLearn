...
public class AccesoBdatos {
	...
	public boolean insertar(Integer clave, String nombre, String localidad) {
		try {
			...
			// El primer ? de la instrucci�n de la l�nea 37 es un entero y su valor se
			// obtiene del argumento "clave" pasado al m�todo insertar
			inserta.setInt(1, clave);
			// El segundo ? de la instrucci�n es una cadena y su valor se obtiene del
			// argumento "nombre" pasado al m�todo insertar
			inserta.setString(2, nombre);
			// El tercer ? de la instrucci�n es una cadena y su valor se obtiene del
			// argumento "localidad" pasado al m�todo insertar
			inserta.setString(3, localidad);
			// Se lleva a cabo la ejecuci�n de la instrucci�n
			inserta.executeUpdate();
			// Nota a la l�nea anterior. Si pusi�ramos en su lugar int filas =
			// inserta.executeUpdate();
			// adem�s de ejecutarse la instrucci�n anterior nos devolver�a en la variable
			// "filas" el n�mero de filas insertadas
			// (que como mucho ser�a 1 �No?)
			return true; // Todo ha ido bien
			// Cualquier error en la inserci�n, sin distinci�n del tipo del error, lo atrapo
			// a continuaci�n
		} catch (SQLException e) {
			// Atrapar� cualquier problema en la inserci�n: El departamento ya existe, el
			// n�mero de departamento no puede ser null ...
	...
	//
	//
	// m�todo insertarConBean. Recibe un solo argumento, un objeto departamento, y
	// lo inserta como una fila en la tabla dept de la base de datos demodb
	// Recordad que tengo una clase Depto.java "similar" a la tabla dept con la que
	// estamos trabajando
	// Procedimiento Muy similar al anterior, con una modificaci�n para que en caso
	// que la inserci�n sea correcta devuelva un 1 y
	...
	//
	// M�todo consultarTodos. �NO DEVUELVE UN RESULTSET O CONJUNTO DE FILAS DE UNA
	// TABLA QUE TANTO NOS GUSTA! SINO
	// QUE ESE RESULTSET OBTENIDO, LO PASA A UN ARRAYLIST DE DEPARTAMENTOS Y
	// DEVUELVE DICHO ARRAYLIST
	//
	public ArrayList<Depto> consultarTodos() {
		// Creamos una lista donde ir a�adiendo cada departamento que vamos recuperando
		// del ResultSet
		ArrayList<Depto> deptos = new ArrayList<Depto>();
		try {
			// Preparo la sentencia. Observad que no lleva "?" o datos que tenga que pasarle
			// antes de ejecutarla.
			PreparedStatement consulta = conecta.prepareStatement("SELECT * FROM dept");
			// Ejecuto la instrucci�n y obtengo en reg el ResultSet o conjunto de filas
			// resultantes de ejecutar la Select...
			ResultSet reg = consulta.executeQuery();
			// Paso a recorrer cada una de las filas del ResultSet
			while (reg.next()) {
				// Creo un departamento y a la vez le paso al constructor los datos con los que
				// inicializo dicho departamento
				// Esos 3 datos con los que construyo el departamento son los datos que extraigo
				// de la fila del ResultSet
				Depto departamento = new Depto(reg.getInt(1), reg.getString(2), reg.getString(3));
				// una vez creado el departamento con los datos extra�dos de la fila, lo a�ado a
				// la lista
	...
	//
	// m�todo consultarUno. Le paso como argumento el n�mero de departamento y me
	// devuelve un objeto departamento
	// que se corresponde con dicho n�mero. Si no lo encuentra o se produce error,
	// devuelve nulo
	public Depto consultarUno(int numero) {
		try {
			...
			// es un entero que se obtiene del argumento "numero" enviado al m�todo
			// consultarUno
			consulta.setInt(1, numero);
			// Ejecuto la instrucci�n
			ResultSet reg = consulta.executeQuery();
			// Creo un departamento donde dejar el resultado obtenido. El ResultSet reg solo
			// puede tener una fila o ninguna,
			// pues realizo una b�squeda por clave primaria Select... WHERE deptno=....
			Depto departamento = new Depto();
			// Si he recuperado una fila, monto el departamento con los datos de las
	...
	// m�todo actualizar. Le pasamos un objeto departamento y actualiza en la tabla
	// dept,
	// el nombre y la localidad de la fila cuyo n�mero de departamento coincide con
	// el del departamento que le paso
	...
	public int actualizar(Depto registro) {
		try {
			...
			// ejecuto la instrucci�n "actualiza.executeUpdate()" y despu�s de ello devuelvo
