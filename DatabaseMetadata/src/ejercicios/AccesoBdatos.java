	...
	public void columnasMetadata(String catalogo, String tabla) throws SQLException {
		...
		while (rs.next()) {
			mensaje = mensaje + "Nombre: " + rs.getString("TABLE_NAME") + " - " 
							  ...
							  + "¿Adminte nulos? " + rs.getString("IS_NULLABLE") + "\n";
		}
		if(mensaje.isEmpty()) {
			JOptionPane.showMessageDialog(null, "No existe la BD" + catalogo + " o la tabla " + tabla, "Columnas de la tabla " + tabla + " de la BD" + catalogo, JOptionPane.INFORMATION_MESSAGE);
			System.exit(-1);
		}
		mensaje = mensaje + "-----------------------------------------------------------------------------------------------------------" + "\n";
		...
		JOptionPane.showMessageDialog(null, mensaje, "Columnas de la tabla departamentos de la Bda empleados",
