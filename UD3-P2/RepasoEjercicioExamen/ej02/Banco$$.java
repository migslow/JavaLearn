...
public class Banco$$ {
	...
	public boolean ingresar(long n, double s) {
		for (Cuenta y : lista) {
			if (y.getNumero() == n) {
	...
	public String buscarCuentaPorNumero(long n) {
		String mensaje = " Elemetos de la lista encontrados por " + n + ": ";
		for (Cuenta y : lista) {
			if (y.getNumero() == n) {
				mensaje = mensaje + "\n" + y + "\n------------------------------------------------------------";
			}
	...
	/*
	 * No se puede public void ordenacionComparatorNumeros() { ArrayList<Cuenta> l =
	 * new ArrayList<Cuenta>(); for(Cuenta y : lista) { lista.add(y); }
	 * Collections.sort(lista, new NumeroComparator()); for(Cuenta x : lista) {
	 * System.out.println(x); } }
	 */
}
